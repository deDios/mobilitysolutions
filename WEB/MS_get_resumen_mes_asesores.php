...
try {
  ...
  // Helper para ejecutar consultas con IN dinámico (sin sprintf por los % de LIKE)
  $run = function($sqlBase, $bindTailTypes, $bindTailValues, $onRow) use ($con, $ph, $types_ids, $ids){
    $sql = str_replace('%s', $ph, $sqlBase);
  ...
  $run($sqlNre, "ss", [$start,$end], function($row) use (&$map){
    $u = (int)$row['uid']; if (!isset($map[$u])) return;
